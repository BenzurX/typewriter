...
**The opening is not a panel.** The sheet opening paints no background of its own. Whatever the paper does not cover is the inside of the machine and is drawn as such: the well, the fan of typebars converging on the basket slung below the top of the opening, the slotted segment arc they rise through, and the ribbon running across at the strike line. A flat rectangle behind the paper is a bug: it hides the machine the rest of the work went into building.

**Where the strike point sits.** About 55% of the way down the sheet opening, not near the top. The height of the strike point is the whole of what the writer can re-read: the finished lines occupy the space above it and the unfed paper runs below. Placed high, the machine shows one or two readable lines over fifteen of blank paper, which is the wrong way round - the writer spends the session looking at paper they have not used yet and cannot see the sentence they just finished. Placed at mid-opening it is roughly eight lines each way.

This is not the same as where the typebars converge. A real basket pivots well under the platen and the bars swing up to the paper, so the fan meets at the basket, not at the strike line, and the two heights are set independently and should stay that way.

Note what this does not fix. Because the opening does not clip the sheet, lines that climb past the top of it keep drawing out onto the desk and the platen rail and bail cross them. A lower strike point buys more readable lines before that happens; it does not stop it happening. That is a consequence of the sheet travelling free of the opening, which is deliberate.
