...
<!-- `Codex:` Restored the strike point near the platen and moved the desktop machine down instead, per Ben's revised direction. -->
**Where the strike point sits.** About 21% of the way down the sheet opening, close to the platen and carriage where a real typebar meets the paper. Preserve room to re-read finished lines by placing the whole machine lower in a tall desktop viewport, allowing the fed paper to remain visible above the platen. Short windows and phones stay vertically compact rather than forcing the body and controls off-screen.
...
Because the opening does not clip the sheet, lines that feed above the platen remain visible on the desk until they leave the viewport. The machine's lower position creates that viewing area without separating the strike point from the carriage. The front-face brass bars and maker's plate stay near the top of the hood so lowering the machine does not hide them below the viewport.
